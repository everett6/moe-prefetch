"""
B2 end-to-end: online adaptation through the real prefetch engine.
...
OUT = os.path.join(HERE, "b2_bench_result.json")
ETA = float(os.environ.get("ETA", "0.001"))
...
def main():
    ...
    results = {}
    W_frozen = {k: v.copy() for k, v in pred.weights.items()}
    for policy in ("lru+predict", "lru+predict-online"):
        pred.weights = {k: v.copy() for k, v in W_frozen.items()}
        ...
        for (p, t) in order:
            ...
            for L in range(n_layers):
                ...
                if policy.startswith("lru+predict") and (L in pred.weights) and (L in rows):
                    f = pred._feats(np.array([rows[L]]), L)[0]
                    s = f @ pred.weights[L]
                    ids = np.argpartition(-s, TOP_P)[:TOP_P]
                    cache.prefetch(L + 1, ids)
                    # Online: the router reveals the truth for L+1 a moment later.
                    # Predict first, then learn from it -- never the reverse.
                    if policy.endswith("online") and (L + 1) in rows:
                        y = np.zeros(N_EXPERT, dtype=np.float32)
                        y[E[rows[L + 1]]] = 1.0
                        pred.weights[L] += ETA * np.outer(f, y - s)
    ...
    lru, lp = results["lru+predict"], results["lru+predict-online"]
